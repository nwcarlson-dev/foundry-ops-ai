...
/**
 * Weekly schedule, interactive part.
 *
 ...
 *
 * Seeded with a schedule the server already built, so the first paint needs no
 * fetch at all — the grid is in the HTML. Only changing the department filter
 * goes back to the network, which is the one thing here that genuinely is a
 * client interaction.
 */
...
export type ScheduleData = Schedule & { summary: string | null; summary_pending?: boolean };

export function ScheduleView({ initial }: { initial: ScheduleData }) {
    ...
    useEffect(() => {
        // The server rendered the unfiltered week into the HTML, so there is
        // nothing to fetch until someone picks a department.
        if (dept === '' && !fetchedOnce.current) return;
